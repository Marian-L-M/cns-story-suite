...
function buildOrderedNodes(nodes, edges, startNodeId) {
  ...
  for (const n of nodes) {
    if (!visited.has(n.id)) visit(n.id, 0, false);
  }
  return result;
}
...
function drawStory(canvas, data, activeNodeId) {
  ...
  if (m?.bgType === 'image' && m.bgImageUrl) {
    ...
    if (bg.complete && bg.naturalWidth) ctx.drawImage(bg, 0, 0, W, H);else {
  ...
  if (story.lineStyle === 'dashed') ctx.setLineDash([10, 5]);else if (story.lineStyle === 'dotted') ctx.setLineDash([2, 5]);else ctx.setLineDash([]);
...
function initBlock(blockEl) {
  ...
  function redraw() {
    drawStory(canvas, data, activeNodeId);
  }
  function rerender() {
